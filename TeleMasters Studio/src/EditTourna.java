...
public class EditTourna {
	...
	private void initialize() {
		...
		try {
			// Checks if tournament already started
			tournaStatus = ExcelHandler.readSpecificCell(fileDirectory + "MatchSched.xlsx", 0, 2);
		...
		btnEditTeam.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (tournaStatus.equalsIgnoreCase("WAITING FOR TEAMS")) {
					...
					JOptionPane.showMessageDialog(btnEditTeam, "Changes are not allowed once tournament has started!");
				}
			}
		}); 
		...
		btnSaveEdit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (txtTeamName.getText().isEmpty() || txtField_T1M1.getText().isEmpty()
					...
					JOptionPane.showMessageDialog(btnSaveEdit, "Please input all credentials!");
				} else {
					list.clear();
		...
		btnSaveEdit.setVerticalAlignment(SwingConstants.BOTTOM);
		btnSaveEdit.setForeground(Color.WHITE);
		btnSaveEdit.setFont(new Font("Tungsten Bold", Font.PLAIN, 20));
		btnSaveEdit.setBackground(new Color(189, 57, 68));
		btnSaveEdit.setBounds(885, 255, 119, 39);
